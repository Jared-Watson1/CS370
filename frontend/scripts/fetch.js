...
function addTask_API(taskType) {
  ...
  const taskTitle = document.getElementById(`${taskType === 'DoAFavor' ? 'doTask' : 'getTask'}Title`).value;
  const taskDescription = document.getElementById(`${taskType === 'DoAFavor' ? 'doTask' : 'getTask'}Description`).value;
...
function fetchAndDisplayTasks(taskType) {
  ...
  .then(data => {
    // Handle the API response
    const taskList = document.getElementById(`${taskType === 'DoAFavor' ? 'doTask' : 'getTask'}List`);
